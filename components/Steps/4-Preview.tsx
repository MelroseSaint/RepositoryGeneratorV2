...
import { generateFileTree } from '../../services/mockAiWorker';
...
export const StepPreview: React.FC<StepPreviewProps> = ({ config, rawInput, onNext, onBack }) => {
  const [files, setFiles] = useState<FileNode[]>([]);
  const [selectedFile, setSelectedFile] = useState<FileNode | null>(null);

  useEffect(() => {
    // Generate the tree
    const tree = generateFileTree(config, rawInput);
    setFiles(tree);
  ...
  return (
    <div className="flex flex-col h-full max-w-6xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4">
      <div className="mb-6 flex justify-between items-end">
        ...
        <div className="flex space-x-3">
            <div className="bg-dark-surface px-3 py-1 rounded border border-dark-border text-xs text-gray-400">
                <span className="text-green-400 font-bold">{files.length + 5}</span> files generated
            </div>
        </div>
      </div>
      ...
      <div className="mt-8 flex justify-between items-center">
        ...
        <button
          onClick={onNext}
          className="bg-green-600 hover:bg-green-500 text-white px-8 py-3 rounded-lg font-semibold shadow-lg shadow-green-900/20 flex items-center transition-transform transform hover:-translate-y-0.5"
        >
